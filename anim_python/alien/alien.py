...
randoms = np.random.randint(30, size=(40000))
...
def update(*args):
    ...
    scatter.set_sizes(randoms)

fig = plt.figure(figsize=(9,16), dpi = 120)
ax = plt.subplot(1,1,1, aspect=1, frameon=False)

scatter = plt.scatter(X, Y, s=1.5, edgecolor="none", facecolor="black", alpha=0.75)
...
anim = animation.FuncAnimation(fig, update, frames=nb, interval=20)
pbar = tqdm.tqdm(total=nb)
